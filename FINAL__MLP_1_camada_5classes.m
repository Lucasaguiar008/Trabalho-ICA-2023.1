...
%caminnho_salvar = "C:\Users\Notebook\Desktop\TRABALHO AMORA 3\RNA\30_05\Programa Final 2\Resultados 2 classes\1 camada\Tangente Hiperbólica\";
    ...
    for rodada=1:10
        ...
        for epoca = 1:num_epocas
            ...
            for i = 1:length(X_treinamento)
                ...
                saida = funcao_ativacao(pesos_saida * saida_oculta1 + bias_saida);
                ...
                switch tipo
                    case 1
                        ...
                        delta_oculta1 = (pesos_saida' * delta_saida) .* saida_oculta1 .* (1 - saida_oculta1);
                    case 2
                        % Backpropagation usando a Tangente Hiperbólica
                        delta_saida = erro .* 0.5 .* (1 - saida.^2);
                        delta_oculta1 = (pesos_saida' * delta_saida) .* 0.5 .* (1 - saida_oculta1.^2);
                    case 3
                        % Backpropagation usando a ReLu
                        delta_saida = erro .* derivada_relu(saida);
                        delta_oculta1 = (pesos_saida' * delta_saida) .* derivada_relu(saida_oculta1);
                    otherwise
                ...
                % Atualizando pesos
                pesos_saida = pesos_saida + taxa_aprendizagem * delta_saida * saida_oculta1';
            ...
            for j = 1:length(X_validacao)
                ...
                saida_validacao = funcao_ativacao(pesos_saida * saida_oculta1_validacao + bias_saida);
            ...
            for k = 1:length(X_teste)
                ...
                saida_teste = funcao_ativacao(pesos_saida * saida_oculta1_teste + bias_saida);
        ...
        for k = 1:length(X_validacao)
            d_teste = Y_validacao(:, k); % saída desejada para validação
            entrada_teste = X_validacao(:, k); % entrada de validação

            % Forward pass na validação
            saida_oculta1_teste = funcao_ativacao(pesos_oculta1 * entrada_teste + bias_oculta1);
            saida_teste = funcao_ativacao(pesos_saida * saida_oculta1_teste + bias_saida);

            [~, resultado_validacao(k, 1)] = max(d_teste); % desejado
            [~, resultado_validacao(k, 2)] = max(saida_teste); % obtido
        ...
        novoNome = ["Rodada_"+num2str(rodada)+"_Camada_n1_"+num2str(num_oculta1)+"_Acurácia_valicacao_"+ num2str(acuracia_percentual_validacao,2)+".fig"];

        % Salvar a figura com o novo nome do arquivo
        savefig(novoNome);

        %Criando a tabela de resultados
        rodada_res = ["Rodada: "+num2str(rodada)+" N = "+num2str(num_oculta1)];
